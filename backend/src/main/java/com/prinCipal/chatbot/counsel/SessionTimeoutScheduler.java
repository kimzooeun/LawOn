    ...
    public void checkSessionTimeouts() {
     ...
     // ==========================================
        ...
        for (CounsellingSession session : warningTargets) {
            ...
            // 2) 상태 업데이트
            session.updateWarningSent(true); // 엔티티 필드 변경
            
            // 3) [필수] 명시적 저장 (확실한 반영을 위해 추가)
            sessionRepository.save(session); 
            
            logger.info("⚠️ 세션 ID {} : 5분 경고 발송 완료 & DB 업데이트", session.getSessionId());
        }
        ...
        if (!timeoutTargets.isEmpty()) {
            logger.info("🔍 종료 대상 세션 {}개 발견", timeoutTargets.size());
        }
