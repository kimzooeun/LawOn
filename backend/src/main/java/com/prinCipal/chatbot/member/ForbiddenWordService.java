...
public class ForbiddenWordService {
	...
	// @PostConstruct: Spring이 이 컴포넌트를 초기화한 직후(서버 시작 시) 이 메소드를 자동으로 실행하여 파일을 로드합니다.
	@PostConstruct
	public void loadWords() {
		...
		try (BufferedReader reader = new BufferedReader(
			...
			// Set에 저장합니다.
			this.forbiddenWords = reader.lines().map(String::trim).filter(line -> !line.isEmpty())
	...
	public boolean containsForbiddenWord(String text) {
		...
		return forbiddenWords.stream().anyMatch(forbiddenWord -> lowerText.contains(forbiddenWord));
	}
