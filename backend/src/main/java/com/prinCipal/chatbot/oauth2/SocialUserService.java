...
public class SocialUserService {
	...
	public Member saveOrUpdateSocialMember(SocialUserInfo socialUserInfo, String provider) {
		...
		if (existingUserOpt.isPresent()) {
			...
			existingUser.updateSocialInfo(socialUserInfo.getName(), socialUserInfo.getProfileImageUrl() // 새 프로필 이미지
			);
			return this.memberRepository.save(existingUser);
		} else {
			Member newMember = Member.builder()
					// 닉네임이 중복될 수 있기 때문에, 이메일 기반으로 유니크한 닉네임을 자동 생성하기 위한 설계
					.nickname(generateUniqueNickname(socialUserInfo.getEmail())).displayName(socialUserInfo.getName())
					// 실제로 후에 , 이메일을 받게된다면
					// email(socialUserInfo.getEmail() != null ? socialUserInfo.getEmail() :
					// generateTempEmail(socialUserInfo.getName()));
					.password(passwordEncoder.encode(UUID.randomUUID().toString())).role(UserRole.USER)
					.socialProvider(provider).socialId(socialId).profileImageUrl(socialUserInfo.getProfileImageUrl())
					.build();
