...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http, JwtAuthenticationFilter jwtAuthenticationFilter,
		...
		return http.csrf((csrf) -> csrf.disable()).cors(cors -> cors.configurationSource(request -> {
			...
			return config;
				...
				.addFilterAt(customLoginFilter, UsernamePasswordAuthenticationFilter.class) // 일반 로그인(JSON) 처리를 위한 커스텀
																							// 필터 등록
				.build();
	}
	...
	// (인증 요청을 처리할 중심)
	// AuthenticationManager는 여러 AuthenticationProvider를 사용하여 인증을 시도하는데 주로
	...
	public JwtAuthenticationFilter jwtAuthenticationFilter(JwtTokenProvider jwtTokenProvider,
			MemberService memberService, BlackTokenRepository blackTokenRepository) {
		return new JwtAuthenticationFilter(jwtTokenProvider, memberService, blackTokenRepository);
	}

	@Bean
	public CustomAuthenticationFilter customAuthenticationFilter(AuthenticationManager authenticationManager,
			LocalJwtAuthenticationSuccessHandler jwtSuccessHandler,
			LocalJwtAuthenticationFailureHandler jwtFailureHandler) {
